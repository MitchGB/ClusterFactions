...
public class MainPermissionMenu extends InventoryBase{
	...
	public MainPermissionMenu(Player player) {
		...
		this.setItem(new ItemBuilder(ItemRepo.GUI_BASE).coloredName("&7Faction Permissions").coloredLore(" ", " ", "&aClick Me").create(), e -> {new FactionPermissionMenu(player).openInventory(player);}, 0, 1, 2, 3, 9, 10, 11, 12, 18, 19, 20, 21);
